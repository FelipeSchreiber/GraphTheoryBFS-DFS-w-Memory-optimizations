...
char* geraVetorAdjacencia(FILE *arq,char *Linha,char *result, int numVertices, int *numArestas, int *vetorGraus){
    char *MatrizVertice;
    int i,pri,seg,ultimoCaracter;
//    int j;
    
    MatrizVertice = bitVectorCreator(numVertices);
    ...
    while (!feof(arq))
    {
        ...
        addVectorBit(MatrizVertice,pri,seg,numVertices);
...
int* BFSVetor(char *Grafo, int s, int tam){
    int v,i;
    ...
    while(!fila_vazia(Q)){
        ...
        for(i=1;i<=tam;i++){
            int w = i;
            if(getVectorBit(Grafo,v,i,tam)){  // Verificar se é adjacente
...
// DFS de Matriz
int* DFSVector(char *Grafo, int s, int tam){
    int v,i;
    Pilha *P = pilha_cria();
    Arvore *vetorArvore = malloc(tam*sizeof(Arvore));
    int *vetorMarcacao = malloc(tam*sizeof(int));
