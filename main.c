...
int main()
{
    ...
    arq = fopen("Input", "rt");// ArqTeste   as_graph   dblp     live_journal
    ...
    if(escolha == 'M' || escolha == 'm'){
        start = getMicrotime();
        MatrizVertice = geraVetorAdjacencia(arq,Linha,result,numVertices,&numArestas, vetorGraus);
        //geraMatrizAdjacencia(arq,Linha,result,numVertices,&numArestas, vetorGraus);
        end = getMicrotime();
...
//    printf("Diametro: %d\n",o);
    ...
    //bitVectorPrint(MatrizVertice,numVertices);
    //printf("\nbitVector[0]:%d bitVector[1]:%d \n",MatrizVertice[0],MatrizVertice[1]);
